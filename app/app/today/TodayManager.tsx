              ...
              <article className={`ruutin-card ruutin-profile-card ruutin-profile-card-tone-${profileIndex % 4}`} key={profile.id}>
                ...
                {profile.tasks.length > 0 && <div className="ruutin-today-task-list" aria-label={`${profile.nickname}'s routines`}>
                  {profile.tasks.map((task) => <div className="ruutin-today-task" key={task.id}>
                    ...
                    {task.state !== "todo" && <span className={`ruutin-task-state ${task.state}`} aria-label={taskStateLabel(task)} title={taskStateLabel(task)}><span aria-hidden="true">{taskStateGlyph(task)}</span></span>}
                    {task.state === "todo" && !profile.archivedAt && <button className="ruutin-button ruutin-parent-done-button" type="button" aria-label={busyKey === `complete-${task.id}` ? `Logging ${task.title} for ${profile.nickname}` : `Mark ${task.title} done for ${profile.nickname}`} aria-busy={busyKey === `complete-${task.id}`} title={busyKey === `complete-${task.id}` ? "Saving…" : "Mark routine done"} disabled={busyKey !== ""} onClick={() => void act(`complete-${task.id}`, () => parentRequest("/api/parent/completions", { profileId: profile.id, taskId: task.id, dueDate: task.dueDate }), `Logged for ${profile.nickname} — ${task.stars} ${task.stars === 1 ? "star" : "stars"} added to their balance.`)}>{busyKey === `complete-${task.id}` ? <span className="ruutin-inline-spinner" aria-hidden="true" /> : <span className="ruutin-parent-done-check" aria-hidden="true" />}</button>}
                  </div>)}
                </div>}
