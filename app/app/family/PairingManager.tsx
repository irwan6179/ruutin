...
export function PairingManager({
  ...
  return (
    <>
      ...
      <section className="ruutin-card ruutin-device-manager" id={deviceSectionId} aria-labelledby="devices-title"><div className="ruutin-section-heading"><div><p className="ruutin-eyebrow">Linked devices</p><h2 id="devices-title">Companion access</h2></div><span className="ruutin-count-pill">{devices.length}</span></div><p className="ruutin-muted-note">Only parents can rename, revoke, or replace a linked device. Revocation takes effect on its next request.</p>{devices.length === 0 ? <p className="ruutin-empty-state">No devices linked.</p> : <ul className="ruutin-simple-list">{devices.map((device) => <li className="ruutin-device-row" key={device.id}><span className="ruutin-avatar small" aria-hidden="true">{device.profileEmoji}</span><span className="ruutin-device-summary"><strong>{device.deviceLabel}</strong><small>{device.profileNickname} · linked {formatDeviceDate(device.createdAt, timezone)} · last active {formatDeviceDate(device.lastSeenAt, timezone, true)}</small><span className={device.revokedAt ? "ruutin-state-note" : "ruutin-state-note good"}>{device.revokedAt ? `Revoked ${formatDeviceDate(device.revokedAt, timezone)}` : "Linked"}</span></span><div className="ruutin-device-actions">{!device.revokedAt && <><label className="sr-only" htmlFor={`device-label-${device.id}`}>Rename {device.deviceLabel}</label><input id={`device-label-${device.id}`} value={labels[device.id] ?? device.deviceLabel} onChange={(event) => setLabels((previous) => ({ ...previous, [device.id]: event.target.value }))} maxLength={40} /><button className="ruutin-button secondary" type="button" onClick={() => void renameDevice(device)} disabled={busy !== ""}>{busy === `rename:${device.id}` ? "Saving…" : "Rename"}</button><button className="ruutin-text-button danger" type="button" onClick={() => void revokeDevice(device)} disabled={busy !== ""}>{busy === `revoke:${device.id}` ? "Revoking…" : "Revoke"}</button></>}{device.revokedAt && <button className="ruutin-button secondary" type="button" onClick={() => void createChallenge(device.profileId)} disabled={busy !== ""}>Create replacement link</button>}</div></li>)}</ul>}</section>
    </>
  );
}
