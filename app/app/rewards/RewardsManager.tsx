...
export function RewardsManager({
  ...
  onActiveRewardChange?: (rewardId: string | null) => void;
  embedded?: boolean;
}) {
  ...
  async function chooseActive(rewardId: string | null) {
    ...
    try { await parentMutation("/api/parent/rewards/active", "POST", { profileId: selectedProfile.id, rewardId }); await refresh(); setActiveRewardIds((current) => ({ ...current, [selectedProfile.id]: rewardId })); onActiveRewardChange?.(rewardId); setNotice(rewardId ? "Active goal updated." : "Active goal cleared."); }
  ...
  return (
    <div className="ruutin-page-stack">
      <ActionPendingOverlay
        ...
        label={refreshing ? "Refreshing rewards…" : busy.startsWith("archive-") ? "Archiving this reward…" : busy.startsWith("request-") ? "Updating the reward request…" : "Saving reward changes…"}
      />
      {!embedded && <section className="ruutin-page-heading" aria-labelledby="rewards-title"><div className="ruutin-page-heading-top"><div><p className="ruutin-eyebrow">Small wins</p><h1 id="rewards-title">Rewards, your way.</h1></div><button className="ruutin-button secondary compact" type="button" onClick={() => void refreshManually()} disabled={Boolean(busy) || refreshing}>{refreshing ? "Refreshing…" : "Refresh"}</button></div><p>Choose a meaningful moment for each person.<span className="sr-only"> Rewards are not a shop or a competition.</span></p></section>}
      <section className="ruutin-card ruutin-rewards-toolbar" aria-label="Reward profile selection"><label htmlFor="reward-profile">Reward profile</label><select id="reward-profile" value={selectedProfile.id} onChange={(event) => { setProfileId(event.target.value); setEditor(null); setError(""); void refresh(event.target.value); }}>{activeProfiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.emoji} {profile.nickname}</option>)}</select><span className="ruutin-state-note ruutin-reward-capacity">{activeRewards.length} of 5 reward ideas active</span><button className="ruutin-button" type="button" onClick={startCreate} disabled={activeRewards.length >= 5}>Add reward idea <span aria-hidden="true">+</span></button></section>
      ...
      <section aria-labelledby="active-rewards-title"><div className="ruutin-section-heading"><div><p className="ruutin-eyebrow">Parent catalogue</p><h2 id="active-rewards-title">Ideas for {selectedProfile.nickname}</h2><p className="ruutin-muted-note">Choose one current goal; the rest stay ready for later.</p></div><span className="ruutin-count-pill">{activeRewards.length}</span></div>{activeRewards.length === 0 ? <p className="ruutin-empty-state">No reward ideas yet. Add one small, meaningful moment to begin.</p> : <div className="ruutin-reward-catalogue">{activeRewards.map((reward) => { const isActive = (activeRewardIds[selectedProfile.id] ?? null) === reward.id; return <article className={`ruutin-card ruutin-reward-card${isActive ? " is-active" : ""}`} key={reward.id} aria-labelledby={`reward-title-${reward.id}`}><header className="ruutin-reward-card-header"><div className="ruutin-profile-card-top"><span className="ruutin-avatar" aria-hidden="true">{reward.emoji}</span><div><h3 id={`reward-title-${reward.id}`} className="ruutin-reward-title" title={reward.title}>{reward.title}</h3><p className="ruutin-reward-cost"><strong>{reward.starCost} {reward.starCost === 1 ? "star" : "stars"}</strong> to unlock this idea</p></div></div><span className={`ruutin-state-note ${isActive ? "good" : ""}`}>{isActive ? "Current goal" : "Available idea"}</span></header><p className="ruutin-reward-card-guidance">{isActive ? "Stars are collecting toward this moment." : "Make this the one goal shown on the companion screen."}</p><div className="ruutin-family-actions"><button className={`ruutin-button ${isActive ? "secondary" : ""}`} type="button" onClick={() => void chooseActive(isActive ? null : reward.id)} disabled={Boolean(busy)}>{busy === `active-${reward.id}` ? "Saving…" : isActive ? "Clear current goal" : "Make active goal"}</button><button className="ruutin-button secondary" type="button" onClick={() => startEdit(reward)} disabled={Boolean(busy)}>Edit idea</button><button className="ruutin-text-button danger" type="button" onClick={() => void archive(reward)} disabled={Boolean(busy)}>{busy === `archive-${reward.id}` ? "Archiving…" : "Archive idea"}</button></div></article>; })}</div>}</section>
      ...
      <section className="ruutin-card ruutin-queue-card ruutin-reward-request-queue" aria-labelledby="reward-requests-title"><div className="ruutin-section-heading"><div><p className="ruutin-eyebrow">Parent review</p><h2 id="reward-requests-title">Reward requests</h2><p className="ruutin-muted-note">A request stays pending until you choose what happens next.</p></div><span className="ruutin-count-pill">{pendingRequests.length}</span></div>{pendingRequests.length === 0 ? <p className="ruutin-empty-state">No requests waiting. When {selectedProfile.nickname} asks to use their stars, the request will appear here.</p> : <ul className="ruutin-simple-list">{pendingRequests.map((request) => <li className="ruutin-claim-row" key={request.id}><span className="ruutin-avatar small" aria-hidden="true">{request.rewardEmoji}</span><span className="ruutin-reward-request-copy"><strong>{request.rewardTitle}</strong><small>{request.nickname} · {request.starCost} {request.starCost === 1 ? "star" : "stars"} · asked {dateLabel(request.requestedAt)}</small><span className="ruutin-state-note ruutin-reward-request-status">Waiting for your decision</span></span><span className="ruutin-claim-actions"><button className="ruutin-button compact" type="button" disabled={Boolean(busy)} onClick={() => void decide(request, "approve")}>{busy === `request-${request.id}` ? "Saving…" : "Approve request"}</button><button className="ruutin-text-button danger compact" type="button" disabled={Boolean(busy)} onClick={() => void decide(request, "reject")}>Not now</button></span></li>)}</ul>}</section>
      {historyRequests.length > 0 && <section className="ruutin-card ruutin-queue-card ruutin-reward-history" aria-labelledby="reward-history-title"><div className="ruutin-section-heading"><div><p className="ruutin-eyebrow">A clear record</p><h2 id="reward-history-title">Request history</h2><p className="ruutin-muted-note">Previous decisions stay visible for context.</p></div><span className="ruutin-count-pill">{historyRequests.length}</span></div><ul className="ruutin-simple-list">{historyRequests.map((request) => <li key={request.id}><span className="ruutin-avatar small" aria-hidden="true">{request.rewardEmoji}</span><span className="ruutin-reward-request-copy"><strong>{request.rewardTitle}</strong><small>{request.nickname} · {request.starCost} {request.starCost === 1 ? "star" : "stars"} · {request.status === "approved" ? "approved" : "decided"} {dateLabel(request.resolvedAt ?? request.requestedAt)}</small></span><span className={`ruutin-state-note ${request.status === "approved" ? "good" : ""}`}>{requestLabel(request.status)}</span></li>)}</ul></section>}
