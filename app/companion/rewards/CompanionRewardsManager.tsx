...
export function CompanionRewardsManager({ initialRewards, initialRequests }: { initialRewards: RewardsData; initialRequests: RewardRequest[] }) {
  ...
  return (
    <div className="ruutin-page-stack companion-page-stack">
      ...
      />
      <section className="ruutin-page-heading companion-rewards-heading" aria-labelledby="companion-rewards-title"><div className="ruutin-page-heading-top"><div><p className="ruutin-eyebrow">Rewards</p><h1 id="companion-rewards-title">Pick a reward</h1><span className="sr-only">Your parent chooses the ideas.</span></div><div className="companion-heading-actions"><a className="companion-header-balance" href="#companion-balance-card" aria-label={`Your star balance — ${rewards.balance} stars`}>{rewards.balance} <span aria-hidden="true">✦</span></a><button className="ruutin-button secondary compact" type="button" onClick={() => void refreshManually()} disabled={Boolean(busy) || refreshing}>{refreshing ? "Refreshing…" : "Refresh"}</button></div></div></section>
      <section id="companion-balance-card" className="ruutin-card companion-balance-card companion-active-goal" aria-label="Star balance and active goal"><span className="ruutin-eyebrow">Your stars</span><strong>{rewards.balance} ✦</strong>{rewards.activeReward ? <><p>{rewards.activeReward.emoji} {rewards.activeReward.title} · {rewards.activeReward.starCost} stars</p><div className="ruutin-progress" role="progressbar" aria-label={`Progress towards ${rewards.activeReward.title}`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress}><span aria-hidden="true" style={{ width: `${progress}%` }} /></div><div className="ruutin-card-meta companion-goal-status"><span>{progress}% of the goal</span><strong className={`ruutin-state-note ${activeGoalStatusClass}`} aria-live="polite">{activeGoalStatus}{remaining > 0 && !activeRequest && <span className="sr-only"> ({remaining} more to go)</span>}</strong></div></> : <p>No active goal yet.</p>}</section>
      <section aria-labelledby="reward-list-title"><div className="ruutin-section-heading"><div><p className="ruutin-eyebrow">From your parent</p><h2 id="reward-list-title">Pick a reward</h2></div><span className="ruutin-count-pill">{rewards.rewards.length}</span></div>{rewards.rewards.length === 0 ? <p className="ruutin-empty-state">No rewards yet.</p> : <div className="ruutin-reward-catalogue companion-reward-catalogue">{rewards.rewards.map((reward) => { const pending = pendingByReward.get(reward.id); return <article className={`ruutin-card ruutin-reward-card${reward.isActive ? " is-active" : ""}${pending ? " is-request-pending" : ""}`} key={reward.id}><div className="ruutin-profile-card-top"><span className="ruutin-avatar" aria-hidden="true">{reward.emoji}</span><div><h3>{reward.title}</h3><p>{reward.starCost} {reward.starCost === 1 ? "star" : "stars"}{reward.isActive ? " · active" : ""}</p></div></div>{renderRewardAction(reward)}</article>; })}</div>}</section>
      <section className="ruutin-card ruutin-queue-card" aria-labelledby="companion-request-history-title" aria-label="A clear record of requests"><div className="ruutin-section-heading"><div><p className="ruutin-eyebrow">Activity</p><h2 id="companion-request-history-title">Your requests</h2></div><span className="ruutin-count-pill">{history.length}</span></div>{history.length === 0 ? <p className="ruutin-empty-state">Requests will show up here.</p> : <ul className="ruutin-simple-list">{history.map((request) => { const reward = rewards.rewards.find((item) => item.id === request.rewardId); return <li className={`companion-request-record is-${request.status}`} key={request.id}><span className="ruutin-avatar small" aria-hidden="true">{reward?.emoji ?? "✦"}</span><span><strong>{reward?.title ?? "Reward idea"}</strong><small>{dateLabel(request.resolvedAt ?? request.requestedAt)}</small></span><span className={`ruutin-state-note ${statusClass(request.status)}`}>{statusLabel(request.status)}</span></li>; })}</ul>}</section>
